...
func LoadCustomerByPhone(usex models.UserSession) string {
	//default status
	cus := rpch.GetCusByPhone(usex.Params, usex.Shop.ID.Hex())
...
func main() {
	var port int
	var debug bool
	flag.IntVar(&port, "port", 9887, "help message for flagname")
	...
	log.SetOutputFile(fmt.Sprintf("adminCustomer-"+strconv.Itoa(port)), logLevel)
